#!/bin/bash
# 推送红丝绒 AD 作业到手机 MAA copilot 目录，并生成 task_list.json / config.json
set -euo pipefail

ADB="${ADB:-adb -s emulator-5554}"
PKG=com.aliothmoon.maameow
REMOTE_DIR="/storage/emulated/0/Android/data/${PKG}/files/Maa/copilot"
SRC_DIR="${1:-/root/.config/maa/copilot}"
MANIFEST="${2:-/tmp/arknights-ad-jobs/manifest.json}"
...
for stage in order:
    ...
    if stage in manifest:
        prts_id = manifest[stage].get("id", 0)
    code = stage.replace("AD-EX-", "AD-EX-").replace("AD-", "AD-")
    items.append({
        "name": stage,
